...
class CryptoDES{
	...
	private $IP= array(58,50,42,34,26,18,10,2,60,52,44,36,28,20,12,4,62,54,46,38,30,22,14,6,64,56,48,40,32,24,16,8);
	private $r0idx = array(57,49,41,33,25,17,9,1,59,51,43,35,27,19,11,3,61,53,45,37,29,21,13,5,63,55,47,39,31,23,15,7);
	private $PC1 = array(57,49,41,33,25,17,9,1,58,50,42,34,26,18,10,2,59,51,43,35,27,19,11,3,60,52,44,36);
	private $PC2 = array(14,17,11,24,1,5,3,28,15,6,21,10,23,19,12,4,26,8,16,7,27,20,13,2,41,52,31,37,47,55,30,40,51,45,33,48,44,49,39,56,34,53,46,42,50,36,29,32);
	private $E = array(32,1,2,3,4,5,4,5,6,7,8,9,8,9,10,11,12,13,12,13,14,15,16,17,16,17,18,19,20,21,20,21,22,23,24,25,24,25,26,27,28,29,28,29,30,31,32,1);
	...
	//sebuah array yang memetakan nilai biner ke integer, dengan kunci '0000' sampai '1111' dan nilai 2 sampai 17.
	...
	//sebuah array yang memetakan nilai biner ke integer, dengan kunci '00' sampai '11' dan nilai 0 sampai 3.
	private $sboxkiri = array('00'=>0,'01'=>1,'10'=>2,'11'=>3);

	//sebuah array dari integer, dengan nilai berkisar dari 1 sampai 32.
	private $pbox = array(16,7,20,21,29,12,28,17,1,15,23,26,5,18,31,10,2,8,24,14,32,27,3,9,19,13,30,6,22,11,4,25);

	//sebuah array dari integer, dengan nilai berkisar dari 1 sampai 64.
    ...
    //Method __construct memasukkan nilai dari properti $log ke 1 jika parameter $debugmode tidak sama dengan 0, dan 0 jika tidak.
	function __construct($debugmode=0){
		$this->log = ($debugmode==0) ? 0 : 1;
	}

	// Method consolelog digunakan untuk keperluan pengembangan, dan melog isi dari parameter $data ke console browser jika properti $log bernilai 1.
	//untuk keperluan DEV
	...
	// Method concatenateBIN menggabungkan elemen-elemen dari array $val menjadi sebuah string.
	...
	// Method alatxor melakukan operasi XOR pada dua parameter $val1 dan $val2, dan mengembalikan nilai "0" jika keduanya sama dan "1" jika berbeda.
	function alatxor($val1,$val2){
		return ($val1==$val2) ? "0" : "1";
	}

	// Method pecahtxt membagi string $text menjadi array dari substring-substring dengan panjang $split.
	...
	function enkripsiDES($val,$key){
		...
		for ($i=0; $i < 16; $i++){
			array_push($kpc2, implode('',$this->IPdt($lc[$i].$ld[$i],$this->PC2)));
		}
		...
		for ($i=0; $i < 16; $i++){
			...
			//Ekspansi Data
			$initE = implode('',$this->IPdt($temp_R,$this->E));
			$temp_xor = "";
	...
	function dekripsiDES($val,$key){
		...
		// Inisialisasi variabel $temp_L_arr sebagai array kosong.
		$temp_L = $this->concatenateBIN($l0);
		$temp_R = $this->concatenateBIN($r0);

		// Inisialisasi variabel $FINALR sebagai array kosong.
		$temp_L_arr = array();
		$FINALR = array();
